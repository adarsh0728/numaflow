...
pub async fn run() -> Result<()> {
    ...
    // Initialize Runtime
    let runtime = Runtime::new(daemon_addr.as_str())
        .map_err(|e| println!("error initializing runtime: {:?}", e));
        ...
        CustomResourceType::MonoVertex(config) => {
            ...
            if let Err(e) = monovertex::start_forwarder(cln_token, &config).await {
                if let Error::Grpc(e) = e {
                    ...
                    if let Err(err) = runtime.unwrap().persist_application_error(e).await {
